...
export const useGetFields = () => useMemo(() => {
    ...
    const passwordField = createInput(
        [required, maxLength40, minLength8], 'password', 'password', 'Password*'
    )
    const apiKeyField = createInput(
        [], 'password', 'cookie', 'API key (optional)'
    )
    const rememberMeField = createCheckBox('rememberMe', 'Remember me')
    const captchaField = createInput(
        [required], 'text', 'captcha', 'Please enter captcha'
    )
    return {
