...
export const loadMessagesByDate =
    (date: string = getDate()): AsyncThunk =>
...
export const loadMoreMessages = (): AsyncThunk => async (dispatch, getState) => {
    ...
    const { totalCount: totalCountInResponse } = await dialogsService.requireLastMessage(
        dialogUserId
    );
    ...
    try {
        ...
        const messagesResponse = await dialogsService.requireMessages(
            dialogUserId,
            count,
            pageNumber
        );
...
export const sendNewMessage =
    (formData: NewMessageFormData): AsyncThunk =>
    async (dispatch, getState) => {
        const id = getState().messagesPage.dialogUserId;
        try {
