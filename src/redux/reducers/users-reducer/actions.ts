...
export const usersActions = {
    ...
    changePage: (newCurrentPage: number) =>
        ({ type: 'usersReducer/CHANGE_PAGE', newCurrentPage } as const),
    isFetchingFunc: (isFetching: boolean) =>
        ({ type: 'usersReducer/IS_FETCHING', isFetching } as const),
    ...
    addUsersError: (errorText: string) =>
        ({ type: 'usersReducer/ADD_USERS_ERROR', errorText } as const),
    clearUsersError: () => ({ type: 'usersReducer/CLEAR_USERS_ERROR' } as const),
    setIsLoading: (isLoading: boolean) =>
        ({ type: 'usersReducer/SET_IS_LOADING', isLoading } as const)
};
