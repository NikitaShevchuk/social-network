...
export const profileActions = {
    ...
    isPhotoUpdating: (isUpdating: boolean) => ({type: 'profile-reducer/PHOTO_UPDATING', isUpdating} as const),
    setProfileEditMode: (isInEditMode: boolean) => ({type: 'profile-reducer/SET_PROFILE_EDIT_MODE', isInEditMode} as const),
    setSocialMediaEditMode: (isInEditMode: boolean) => ({type: 'profile-reducer/SET_SOCIAL_MEDIA_EDIT_MODE', isInEditMode} as const),

    // Load profile
    photoUploadedSuccessfully: (photos: Photos) => ({type: 'profile-reducer/UPD_PHOTO', photos} as const),
    setUpdatedProfile: (profile: EditProfilePutRequestData) => ({type: 'profile-reducer/UPDATE_PROFILE', profile} as const),
    setIsProfileLoading: (isLoading: boolean) => ({type: 'profile-reducer/SET_IS_PROFILE_LOADING', isLoading} as const),
    ...
    setIsMyProfile: (isMyProfile: boolean) => ({type: 'profile-reducer/SET_IS_MY_PROFILE', isMyProfile} as const),

    // Errors handles
    addLocalError: (localError: string) => ({type: 'profile-reducer/ADD_LOCAL_ERROR', localError} as const),
    addProfileFetchError: (errorText: string) => ({type: 'profile-reducer/ADD_PROFILE_FETCH_ERROR', errorText} as const),
