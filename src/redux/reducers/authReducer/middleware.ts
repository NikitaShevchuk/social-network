...
const _setMyStatusById = (id: number): AsyncThunk => async (dispatch) => {
...
export const authorize = (): AsyncThunk => async (dispatch) => {
    try {
        ...
        if (authResponse.resultCode === ResultCodes.Success) {
            ...
            await dispatch(_setMyProfileById(id))
            await dispatch(_setMyStatusById(id))
            dispatch(authActions.toggleIsAuthorized(true))
        }
    } catch {
        console.log('Authorization failed. Trying agan...')
...
export const updStatusThunk = (newStatus: string): AsyncThunk => async (dispatch) => {
    try {
